"""
This file runs all of the integration tests in the directory `tests`
"""
import os
import re
import subprocess
...
def gen_executable(file_name):
    ...
    if "Error" in out:
        raise Exception("\n".join(out.split("\n")[1:]))
    subprocess.run(
        "gcc -std=c99 -fsanitize=address -fsanitize=undefined .out/out.c -o .out/a.out",
        shell=True,
        check=True,
...
def get_output(file_name):
    ...
    return subprocess.check_output(".out/a.out", shell=True).decode("utf-8")
